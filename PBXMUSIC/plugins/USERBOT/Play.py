...
from PBXMUSIC.utils.decorators.play import CPlayWrapper
from PBXMUSIC.utils.formatters import formats
...
@Client.on_callback_query(filters.regex("PBXmousAdmin") & ~BANNED_USERS)
async def PBXmous_check(client: Client, CallbackQuery):
    try:
        await CallbackQuery.answer(
...
@Client.on_callback_query(filters.regex("PBXPlaylists") & ~BANNED_USERS)
@languageCB
...
import config
from PBXMUSIC import Carbon, YouTube
from PBXMUSIC.core.call import PBX
...
from PBXMUSIC.utils.inline import aq_markup, close_markup, stream_markup2
...
async def stream(
    ...
    if not result:
        return
    if forceplay:
        await PBX.force_stop_stream(chat_id)
    if streamtype == "playlist":
        ...
        if await is_active_chat(chat_id):
            ...
            if not forceplay:
                db[chat_id] = []
            await PBX.join_call(chat_id, original_chat_id, file_path, video=None)
            await put_queue(
                chat_id,
                original_chat_id,
                file_path,
        ...
        if await is_active_chat(chat_id):
            ...
            if not forceplay:
                db[chat_id] = []
            await PBX.join_call(chat_id, original_chat_id, file_path, video=status)
            await put_queue(
                chat_id,
                original_chat_id,
                file_path,
