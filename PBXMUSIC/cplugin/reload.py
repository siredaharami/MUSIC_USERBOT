...
from PBXMUSIC.utils.decorators import ActualAdminCB, AdminActual, language
from PBXMUSIC.utils.formatters import alpha_to_int, get_readable_time
from PBXMUSIC.mongo.afkdb import LOGGERS
...
async def restartbot(client, message: Message, _):
    ...
    try:
        db[message.chat.id] = []
        await PBX.stop_stream_force(message.chat.id)
    except:
        pass
    ...
    if chat_id:
        ...
        try:
            db[chat_id] = []
            await PBX.stop_stream_force(chat_id)
        except:
            pass
